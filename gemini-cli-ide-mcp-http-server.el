;;; gemini-cli-ide-mcp-http-server.el --- HTTP server for MCP tools  -*- lexical-binding: t; -*-

;; Copyright (C) 2025

;; Author: Enze Chi
;; Keywords: ai, gemini, mcp, http
...
;; context-aware operations for multiple concurrent Gemini sessions.
...
(unless (featurep 'web-server)
  (condition-case err
      (require 'web-server)
    (error
     (gemini-cli-ide-debug "Failed to load web-server package: %s" (error-message-string err)))))
...
(defvar gemini-cli-ide-mcp-http-server--server nil
  "The web-server instance.")


;;; Helper Functions

(defun gemini-cli-ide-mcp-http-server--extract-session-id-from-path (headers)
...
Returns the session ID or nil if not found."
  (let* ((url (or (cdr (assoc :POST headers))
         ...
         (_ (when url (gemini-cli-ide-debug "MCP request URL: %s" url))))
...
(defun gemini-cli-ide-mcp-http-server-start (&optional port)
...
Returns a cons cell of (server . port)."
  ...
  (gemini-cli-ide-debug "Attempting to start MCP server on port %s" (or port "auto"))
  (condition-case err
      (let* ((selected-port (or port 0))  ; 0 means auto-select
             (server (ws-start
                      `(((:GET . "^/mcp\\(/.*\\)?$") . ,#'gemini-cli-ide-mcp-http-server--handle-get)
                        ((:POST . "^/mcp\\(/.*\\)?$") . ,#'gemini-cli-ide-mcp-http-server--handle-post))
                      selected-port
                      nil  ; log buffer
                      :host "127.0.0.1")))  ; local only
        (setq gemini-cli-ide-mcp-http-server--server server)
    ...
    (error
     (gemini-cli-ide-debug "Failed to start web server: %s" (error-message-string err))
     (signal 'error (list (format "Failed to start web server: %s" (error-message-string err)))))))

(defun gemini-cli-ide-mcp-http-server-stop (server)
  "Stop the MCP HTTP server SERVER."
  (when server
    (ws-stop server)
    (setq gemini-cli-ide-mcp-http-server--server nil)))

;;; Request Handlers

(defun gemini-cli-ide-mcp-http-server--handle-get (request)
...
(defun gemini-cli-ide-mcp-http-server--handle-post (request)
...
with the appropriate session context."
  (gemini-cli-ide-debug "MCP server received POST request")
  (condition-case err
      (let* ((headers (ws-headers request))
             (body (ws-body request))
             ;; Extract session ID from URL path
             (url-session-id (gemini-cli-ide-mcp-http-server--extract-session-id-from-path headers))
        ...
        (if (null id)
            ...
            (progn
              (gemini-cli-ide-debug "Received notification: %s" method)
              ;; Still close the connection for HTTP transport
              (gemini-cli-ide-mcp-http-server--send-empty-response request))
          ;; Process the request with session context
          (let* ((gemini-cli-ide-mcp-server--current-session-id url-session-id)
                 (result (gemini-cli-ide-mcp-http-server--dispatch method params)))
            (gemini-cli-ide-debug "MCP response result computed")
            ;; Send response
            (gemini-cli-ide-mcp-http-server--send-json-response
             ...
             `((jsonrpc . "2.0")
               (id . ,id)
               (result . ,result)))
            (gemini-cli-ide-debug "MCP response sent"))))
    ...
    (quit
     (gemini-cli-ide-debug "Request cancelled by user (C-<escape>)")
     (gemini-cli-ide-mcp-http-server--send-json-error
...
(defun gemini-cli-ide-mcp-http-server--dispatch (method params)
  "Dispatch MCP method calls.
METHOD is the JSON-RPC method name.
PARAMS is the parameters alist."
  (pcase method
    ("initialize"
     (gemini-cli-ide-mcp-http-server--handle-initialize params))
    ("tools/list"
     (gemini-cli-ide-mcp-http-server--handle-tools-list params))
    ("tools/call"
     (gemini-cli-ide-mcp-http-server--handle-tools-call params))
    (_
     (signal 'json-rpc-error (list -32601 "Method not found")))))

(defun gemini-cli-ide-mcp-http-server--handle-initialize (_params)
  "Handle the initialize method."
  `((protocolVersion . "2024-11-05")
    (capabilities . ((tools . ((listChanged . :json-false)))
                     (logging . ,(make-hash-table :test 'equal))))
    (serverInfo . ((name . "gemini-cli-ide-mcp-tools")
                   (version . "0.1.0")))))

(defun gemini-cli-ide-mcp-http-server--handle-tools-list (_params)
  "Handle the tools/list method."
  (let ((tools (mapcar (lambda (spec)
                         (gemini-cli-ide-mcp-http-server--tool-to-mcp
                          (gemini-cli-ide--normalize-tool-spec spec)))
                       gemini-cli-ide-mcp-server-tools)))
    (gemini-cli-ide-debug "MCP server returning %d tools" (length tools))
    (dolist (tool tools)
      (gemini-cli-ide-debug "  Tool: %s" (alist-get 'name tool)))
    `((tools . ,tools))))

(defun gemini-cli-ide-mcp-http-server--handle-tools-call (params)
  ...
  (let* ((tool-name (alist-get 'name params))
         ...
         (tool-spec (cl-find-if
                     (lambda (spec)
                       (let ((normalized (gemini-cli-ide--normalize-tool-spec spec)))
    ...
    (let* ((normalized (gemini-cli-ide--normalize-tool-spec tool-spec))
           ...
           (args (gemini-cli-ide-mcp-http-server--validate-args tool-args arg-specs)))
...
(defun gemini-cli-ide-mcp-http-server--tool-to-mcp (tool-spec)
  ...
  (let* ((name (or (plist-get tool-spec :name)
    ...
    `((name . ,name)
      ...
      (inputSchema . ((type . "object")
                      (properties . ,(gemini-cli-ide-mcp-http-server--args-to-schema args))
                      (required . ,(gemini-cli-ide-mcp-http-server--required-args args)))))))

(defun gemini-cli-ide-mcp-http-server--args-to-schema (args)
...
(defun gemini-cli-ide-mcp-http-server--required-args (args)
...
(defun gemini-cli-ide-mcp-http-server--validate-args (args arg-specs)
...
(defun gemini-cli-ide-mcp-http-server--format-result (result)
  "Format RESULT for display to Gemini."
  (cond
...
(defun gemini-cli-ide-mcp-http-server--send-json-response (request status body)
...
(defun gemini-cli-ide-mcp-http-server--send-empty-response (request)
...
(defun gemini-cli-ide-mcp-http-server--send-json-error (request id code message)
...
(provide 'gemini-cli-ide-mcp-http-server)
;;; gemini-cli-ide-mcp-http-server.el ends here
