;;; gemini-cli-ide-mcp-handlers.el --- MCP tool handlers for Gemini Cli IDE  -*- lexical-binding: t; -*-
...
;; This file contains the MCP tool handler implementations for Gemini Cli IDE.
;; These handlers implement the actual functionality that Gemini can invoke
...
(declare-function gemini-cli-ide-mcp-complete-deferred "gemini-cli-ide-mcp" (session method result &optional unique-key))
(declare-function gemini-cli-ide-mcp--get-current-session "gemini-cli-ide-mcp" ())
(declare-function gemini-cli-ide-mcp--get-session-for-project "gemini-cli-ide-mcp" (project-dir))
(declare-function gemini-cli-ide-mcp--get-buffer-project "gemini-cli-ide-mcp" ())
(declare-function gemini-cli-ide-mcp-session-active-diffs "gemini-cli-ide-mcp" (session))
(declare-function gemini-cli-ide-mcp-session-original-tab "gemini-cli-ide-mcp" (session))
(declare-function gemini-cli-ide-mcp--setup-buffer-cache-hooks "gemini-cli-ide-mcp" ())
(declare-function gemini-cli-ide--get-buffer-name "gemini-cli-ide" (&optional directory))
(declare-function gemini-cli-ide--display-buffer-in-side-window "gemini-cli-ide" (buffer))
(defvar ediff-control-buffer)
(defvar ediff-window-setup-function)
(defvar ediff-split-window-function)
(defvar ediff-control-buffer-suffix)
(defvar gemini-cli-ide-mcp--sessions)
...
(defvar gemini-cli-ide-mcp-tools nil
  "Alist of tool names to handler functions.")

(defvar gemini-cli-ide-mcp-tool-schemas nil
  "Tool input schemas for MCP protocol.")

(defvar gemini-cli-ide-mcp-tool-descriptions nil
  "Tool descriptions for MCP protocol.")

;;; Constants

(defconst gemini-cli-ide-mcp-handlers-idle-timer-delay 0
  "Delay in seconds for idle timer when deferring responses.")

;;; Helper Functions

(defun gemini-cli-ide-mcp--find-session-for-file (file-path)
...
(defun gemini-cli-ide-mcp--find-gemini-side-window ()
  "Find the Gemini Cli side window in the current frame.
Returns the window if found, nil otherwise."
  (let ((gemini-buffer-name (gemini-cli-ide--get-buffer-name)))
...
(defun gemini-cli-ide-mcp--get-active-diffs (&optional session)
...
(defun gemini-cli-ide-mcp--create-diff-buffers (old-file-path new-file-contents tab-name)
...
(defun gemini-cli-ide-mcp--setup-diff-hooks (tab-name session saved-winconf)
...
Returns a cons cell (before-setup-hook-fn . startup-hook-fn)."
  (let* ((captured-tab-name tab-name)
    ...
    (setq startup-hook-fn
          (lambda ()
            (gemini-cli-ide-mcp--handle-ediff-startup
             captured-tab-name captured-session captured-winconf startup-hook-fn)))

    (cons before-setup-hook-fn startup-hook-fn)))

(defun gemini-cli-ide-mcp--handle-ediff-startup (tab-name session saved-winconf startup-hook-fn)
  ...
  (when ediff-control-buffer
    ...
    (with-current-buffer ediff-control-buffer
      ...
      (setq-local ediff-quit-hook
                  (list (lambda ()
                          ;; Check if this quit was initiated by Gemini
                          (let* ((active-diffs (gemini-cli-ide-mcp--get-active-diffs session))
                                 (diff-info (gethash tab-name active-diffs))
                                 (quit-from-gemini (alist-get 'quit-from-gemini diff-info)))
                            (unless quit-from-gemini
                              ;; Only handle quit if not initiated by Gemini
                              (gemini-cli-ide-mcp--handle-ediff-quit
                               tab-name
                               session)))
    ...
    (let ((original-window (selected-window))
          (gemini-window nil))
      ;; Restore Gemini side window only if user wants it shown during ediff
      (when gemini-cli-ide-show-gemini-window-in-ediff
        (when-let* ((gemini-buffer-name (gemini-cli-ide--get-buffer-name))
                    (gemini-buffer (get-buffer gemini-buffer-name)))
          (when (buffer-live-p gemini-buffer)
            ;; Display Gemini buffer in side window and save the window
            (setq gemini-window (gemini-cli-ide--display-buffer-in-side-window gemini-buffer)))))

      ;; Handle focus based on user preference
      (cond
       ;; If user wants Gemini window focus and it's visible, select it
       ((and gemini-cli-ide-focus-gemini-after-ediff gemini-window)
        (select-window gemini-window))
...
(defun gemini-cli-ide-mcp-handle-open-file (arguments)
    ...
    (condition-case err
        (progn
          ...
          (with-current-buffer (current-buffer)
            (gemini-cli-ide-mcp--setup-buffer-cache-hooks))
...
(defun gemini-cli-ide-mcp-handle-get-current-selection (_arguments)
...
(defun gemini-cli-ide-mcp-handle-get-open-editors (_arguments)
...
(defun gemini-cli-ide-mcp-handle-get-workspace-folders (_arguments)
  "Get the current workspace folders (project roots)."
  ;; Return the specific project directory for this MCP instance
  (let ((project-dir (or (gemini-cli-ide-mcp--get-buffer-project)
                         default-directory)))
    `((folders . ,(vconcat (list (expand-file-name project-dir)))))))

(defun gemini-cli-ide-mcp-handle-get-diagnostics (arguments &optional session)
...
Optional SESSION contains the MCP session context."
  (gemini-cli-ide-diagnostics-handler arguments session))

(defun gemini-cli-ide-mcp-handle-save-document (arguments)
  "Save a document.
...
(defun gemini-cli-ide-mcp-handle-close-tab (arguments)
  "Close a tab/buffer.
            ...
            (progn
              ...
              (when-let ((control-buf (alist-get 'control-buffer found-diff-info)))
                (when (buffer-live-p control-buf)
                  ...
                  (puthash tab-name found-diff-info (gemini-cli-ide-mcp-session-active-diffs found-session))
              ...
              (gemini-cli-ide-mcp--cleanup-diff tab-name found-session)
...
(defun gemini-cli-ide-mcp-handle-open-diff (arguments)
...
- `tab_name': Name for the diff tab"
  (let ((old-file-path (alist-get 'old_file_path arguments))
    ...
    (setq session (or (gemini-cli-ide-mcp--find-session-for-file old-file-path)
    ...
    (let* ((saved-winconf (current-window-configuration))
           (buffers (gemini-cli-ide-mcp--create-diff-buffers
      ...
      (let* ((hooks (gemini-cli-ide-mcp--setup-diff-hooks tab-name session saved-winconf))
...
(defun gemini-cli-ide-mcp--handle-ediff-quit (tab-name &optional session)
...
session."
  (let* ((temp-session session)  ; Store the passed session
         (active-diffs (gemini-cli-ide-mcp--get-active-diffs temp-session))
         (diff-info (gethash tab-name active-diffs)))
    (if (not diff-info)
      ...
      (let* ((buffer-B (alist-get 'buffer-B diff-info))
        ...
        (let ((accept-changes (y-or-n-p "Accept the changes? ")))
          ...
          (run-with-idle-timer gemini-cli-ide-mcp-handlers-idle-timer-delay nil
                               (lambda ()
                                 (if accept-changes
                                     ;; User accepted changes
                                     (let ((final-content (with-current-buffer buffer-B
                                                            (buffer-string))))
                                       ;; Don't save the file - let Gemini Cli handle the actual file modification
                                       ;; Just return the content that should be saved

                                       ;; Send FILE_SAVED response with the new content
                                       (gemini-cli-ide-mcp-complete-deferred
                                        final-session
                                        "openDiff"
                                       ...
                                       (when final-session
                                         (let ((session-diffs (gemini-cli-ide-mcp--get-active-diffs final-session)))
                                           (puthash tab-name
...
(defun gemini-cli-ide-mcp--cleanup-diff (tab-name &optional session)
      ...
      (let ((buffer-A (alist-get 'buffer-A diff-info))
        ...
        (when (and control-buf (buffer-live-p control-buf))
          (with-current-buffer control-buf
            ...
            (gemini-cli-ide-debug "Capturing ediff buffers from control buffer")
        ...
        (dolist (buf (list error-buffer diff-buffer fine-diff-buffer custom-diff-buffer))
          (when (and buf (buffer-live-p buf))
            (gemini-cli-ide-debug "Killing ediff auxiliary buffer: %s" (buffer-name buf))
            (kill-buffer buf)))
...
(defun gemini-cli-ide-mcp-handle-close-all-diff-tabs (_arguments)
  "Close all diff tabs/buffers for the current session only."
  (let ((closed-count 0)
        (current-session (gemini-cli-ide-mcp--get-current-session)))
    (if current-session
        ;; Only clean up diffs for the current session
        (let ((session-diffs (gemini-cli-ide-mcp-session-active-diffs current-session)))
          (maphash (lambda (tab-name _diff-info)
                     (gemini-cli-ide-mcp--cleanup-diff tab-name current-session)
...
(defun gemini-cli-ide-mcp-handle-check-document-dirty (arguments)
...
(defun gemini-cli-ide-mcp--build-tool-list ()
  "Build the tool list, conditionally including ediff tools."
  `(("openFile" . gemini-cli-ide-mcp-handle-open-file)
    ("getCurrentSelection" . gemini-cli-ide-mcp-handle-get-current-selection)
    ("getOpenEditors" . gemini-cli-ide-mcp-handle-get-open-editors)
    ("getWorkspaceFolders" . gemini-cli-ide-mcp-handle-get-workspace-folders)
    ("getDiagnostics" . gemini-cli-ide-mcp-handle-get-diagnostics)
    ("saveDocument" . gemini-cli-ide-mcp-handle-save-document)
    ("close_tab" . gemini-cli-ide-mcp-handle-close-tab)
    ,@(when (bound-and-true-p gemini-cli-ide-use-ide-diff)
        '(("openDiff" . gemini-cli-ide-mcp-handle-open-diff)
          ("closeAllDiffTabs" . gemini-cli-ide-mcp-handle-close-all-diff-tabs)))
    ("checkDocumentDirty" . gemini-cli-ide-mcp-handle-check-document-dirty)))

(setq gemini-cli-ide-mcp-tools (gemini-cli-ide-mcp--build-tool-list))

(defun gemini-cli-ide-mcp--build-tool-schemas ()
...
(defun gemini-cli-ide-mcp--build-tool-descriptions ()
...
(provide 'gemini-cli-ide-mcp-handlers)

;;; gemini-cli-ide-mcp-handlers.el ends here
